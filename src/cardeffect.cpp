   ...
   void ElementaryEffect::apply(Fighter* attacker, Fighter* defender, Battle* battle, Card& card, int guiChoice)
{
   ...
   bool StrategicDeductionEffect::needsGUIInput() const { return false; }
bool StrategicDeductionEffect::usesHandSelection() const { return false; }
...
void StrategicDeductionEffect::apply(Fighter* attacker, Fighter* defender, Battle* battle, Card& card, int guiChoice)
{
    ...
    if (attackCard == nullptr || defendCard == nullptr) return;
    ...
    if (enemyCard != nullptr) {
        int boostValue = enemyCard->getBoost();
        enemyCard->forceValue(boostValue);
        enemyCard->lockValue(); 
        cout << "Strategic Deduction: " << enemyCard->getName()
             << " value forced to its Boost (" << boostValue << ").\n";
    }
}
